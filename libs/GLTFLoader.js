...
import FileLoader,{TextureLoader} from './FileLoader'
...
var GLTFLoader = ( function () {
	...
	GLTFLoader.prototype = {
		...
		load: function ( url, onLoad, onProgress, onError ) {
			...
			var scope = this;
			APP.log("wx.request:"+url+"  begin!");
			var path = this.path !== undefined ? this.path : THREE.LoaderUtils.extractUrlBase( url );
			APP.log("wx.request:"+url+"  begin!  2");
			wx.request({
				url: url, //仅为示例，并非真实的接口地址
				responseType: "arraybuffer",
				success(res) {
					try {
						APP.log("wx.request:"+url+"  over!");
						scope.parse(res.data, path, onLoad, onError);
					} catch (e) {
				...
				fail(res){
					APP.log("wx.request error:"+JSON.stringify(res)+"  over!");
					onError(JSON.stringify(res));
				}
			})
		},
	...
	GLTFParser.prototype.getMultiDependencies = function ( types ) {
		...
		for ( var i = 0, il = types.length; i < il; i ++ ) {
			...
			var type = types[ i ];
			APP.log("getDependencies:"+( type + ( type === 'mesh' ? 'es' : 's' ) ));
			var value = this.getDependencies( type );
			
			value = value.then( function ( key, value ) {
				APP.log("getDependencies ok:"+key);
				results[ key ] = value;
		...
		return Promise.all( pendings ).then( function () {
			...
			console.error("getMultiDependencies：",e);
			throw new Error("getMultiDependencies：",e)
		});
	...
	GLTFParser.prototype.loadTexture = function ( textureIndex ) {
		...
		return Promise.resolve( sourceURI ).then( function ( sourceURI ) {
			...
			return new Promise( function ( resolve, reject ) {
				...
				loader.load(APP.canvas, resolveURL( sourceURI, options.path ), resolve, undefined, function(e){
					console.log("textureLoader error:"+resolveURL( sourceURI, options.path ));
					APP.log("textureLoader error:"+resolveURL( sourceURI, options.path ));
					reject(e);
				} );
	...
	GLTFParser.prototype.assignTexture = function ( materialParams, textureName, textureIndex ) {
		...
		APP.log("纹理 begin:"+textureName);
		return this.getDependency( 'texture', textureIndex ).then( function ( texture ) {
			
			
			APP.log("纹理 over:"+textureName);
	...
	GLTFParser.prototype.loadMaterial = function ( materialIndex ) {
		...
		APP.log("loadMaterial:"+materialIndex);  
		...
		return Promise.all( pending ).then( function () {
			APP.log("loadMaterial over:"+materialIndex);  
			var material;
